...
@ESBComponent("Apache Async Client")
@Component(service = ApacheAsyncClient.class, scope = PROTOTYPE)
public class ApacheAsyncClient implements ProcessorAsync {
    ...
    public void apply(Message input, FlowContext flowContext, OnResult callback) {
        ...
        ExecutionStrategy.get(method).execute(client, () -> {
                    try {
                        String finalUri = baseURL + uriComponent().expand(pathParameters, queryParameters);
                ...
                () -> Flux.from(input.getContent().asByteArrayStream()),
                HashMap::new,callback, flowContext);
    }
