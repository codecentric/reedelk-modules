...
class RoutesTest {
    ...
    /**
     ...
     @EnumSource(RestMethod.class) void shouldMatchSimplePathWith(RestMethod method) {
     // Given
     ...
     @EnumSource(RestMethod.class) void shouldReturnDefaultRouteWhenNotMatchForSimplePath(RestMethod method) {
     // Given
     Route routeToMatch = new Route(method, "/test", mockHandler);
     Routes routes = createRoutes(routeToMatch);
     ...
     @EnumSource(RestMethod.class) void shouldMatchSimplePathWhenQueryParametersArePresent(RestMethod method) {
     // Given
     Route routeToMatch = new Route(method, "/test", mockHandler);
     Routes routes = createRoutes(routeToMatch);
     ...
     @EnumSource(RestMethod.class) void shouldMatchPathWithVariables(RestMethod method) {
     // Given
     ...
     @ParameterizedTest(name = "Should match path with variables for {0}")
     @EnumSource(RestMethod.class) void shouldMatchPathWithVariablesAndQueryParameters(RestMethod method) {
     // Given
     Route routeToMatch = new Route(method, "/api/users/{code}/{group}", mockHandler);
     Routes routes = createRoutes(routeToMatch);
     ...
     @EnumSource(RestMethod.class) void shouldMatchRootPath(RestMethod method) {
     // Given
